...
v0.16.0, built and published by CI like every release. The version 1 loop (PLAN.md 5.3, M1 to M6), the hardening that followed it (narrowed Process permits, enforced limits, shared images, backups, an end to end job in CI), M7 to M12, and the recipes under `/org/skills` (PLAN.md 2.2) are in. What stays open is in PLAN.md 5.5. Read [PLAN.md](PLAN.md): positioning, the four object model, kits, kitbashOS, releases, and version 1 milestones. Machine readable definitions live in [`spec/`](spec/).
...
base=https://github.com/zyx1121/kitbash/releases/download/v0.16.0
apk=kitbashd-0.16.0-r0.$arch.apk
...
want, with `alpine-sdk` and `go`, as a non root user: `git archive --format=tar.gz --prefix=kitbash-0.16.0/ -o kitbash-0.16.0.tar.gz v0.16.0`
then `sh packaging/apk/build.sh kitbash-0.16.0.tar.gz`. The package lands in
...
**Or boot the ISO.** `kitbash-0.16.0-x86_64.iso` and `kitbash-0.16.0-aarch64.iso`
