...
base=https://github.com/zyx1121/kitbash/releases/download/v0.12.0
apk=kitbashd-0.12.0-r0.$arch.apk
...
want, with `alpine-sdk` and `go`, as a non root user: `git archive --format=tar.gz --prefix=kitbash-0.12.0/ -o kitbash-0.12.0.tar.gz v0.12.0`
then `sh packaging/apk/build.sh kitbash-0.12.0.tar.gz`. The package lands in
...
apk add kitbashd-0.12.0-r0."$(uname -m)".apk
...
**Or boot the ISO.** `kitbash-0.12.0-x86_64.iso` and `kitbash-0.12.0-aarch64.iso`
