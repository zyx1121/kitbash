...
**A Process writing through a mount makes no commit, and that is a known hole.** A unit can declare `mounts` and see folders of Files as bind mounts, see 2.3. A container that writes through a `rw` mount writes into the working tree of that repository directly: the file is there, `fs_read` and `fs_list` see it at once, and `fs_history` does not, because nothing committed it. The repository is left dirty. `fs_write` stages and commits only the paths it names, so what the Process wrote stays untracked, and `pkg_build` of a Package that holds the folder is a `conflict` until the member commits those files or ignores them; a folder a Process writes into should carry a `.gitignore` for what it writes, which is what the `postgres` recipe of 2.2 does. Version 1 accepts this: the alternative is kitbashd committing on behalf of a Process, which needs an author, a message and a policy for a Process that writes a hundred times a minute, and that is a later kit rather than a core change. Nothing is silently hidden: a member reads the Process's files with `fs_read` like any other file.
...
There are no package kinds. A tool server, a workflow engine and an observability kit are all Packages. What distinguishes them is what their manifest declares, not a category the platform maintains. Tags exist for humans to search by. Platform logic never reads tags.

**Skills are folders of knowledge, not Packages.** The core runs any image the same way, and that is the point, but running a model server, a web application or a database well takes choices the core should not make: Ollama for one person switching models, vLLM for many users on a GPU, llama.cpp for one GGUF file, Postgres as a second unit with its data kept out of git. Those choices depend on who asks and change faster than a kitbashd release, so they are written down as skills: a folder whose manifest has a `name` and a `description` and no `deploy` block, holding a `SKILL.md` with the Package to write, the steps and the traps, each one verified on a real host and dated. The description says when to use the recipe and when to use another, because it is the only thing an agent reads before deciding. `/org/skills` is where the shared ones live and is seeded with `serve-ollama`, `serve-llamacpp`, `serve-vllm`, `postgres` and `web-app`; a member's own recipes live in their home and reach `/org` by approval like any write. An agent finds a skill the way it finds anything in Files, by descriptions: `fs_list /org/skills` answers one line per recipe and `fs_read` loads the one it picked, two calls to find and one to read, so a recipe costs nothing until it is wanted. The core has no skill type, no skill tool and no loader, and gains none: a skill is the progressive disclosure of 2.1 applied to know how. The first recipes were written by running them, 2026-09-23: four on the M12 host, and `serve-vllm` from the same image on the aias machine of 5.6, since the M12 host has no GPU. Two of the traps they carry would each have shipped a model server open to the internet: llama-server reads its key from `LLAMA_API_KEY` and silently ignores `LLAMA_ARG_API_KEY`, and Ollama bound to loopback answers `403` through a proxy that forwards the public Host.
...
provides:
  ...
  prompt: USAGE.md            # optional, a file an agent should read before using the tools
...
| Model Context Protocol, 2025-06 revision | The MCP surface | Files map onto MCP primitives: binary content as resources, everything else as tools. A `prompt` entry is a file an agent reads with `fs_read`, not an MCP prompt, because clients offer MCP prompts to the person as commands rather than to the model as context; skills in 2.2 rely on the same reading |
...
- A skill carries pinned digests and measured numbers, and both age. Whether an evaluation kit replays each recipe on a schedule and flags the ones that no longer hold, or a recipe states the version it was verified on and nothing more, is open until the first recipe goes stale. So is how an updated recipe reaches a host that exists: `/org` is seeded once, when a folder is missing, so a release that changes `/org/skills` changes new hosts only.
...
The occasion is concrete. On 2026-09-20 a document OCR model (jina-ocr-v1 under vLLM) went into service for the organization on a Windows machine with one 10 GB GPU, by hand: a systemd unit, a port forward, a firewall rule, a gateway route and a DNS record, none of it a Package and none of it in Telemetry. It was retired on 2026-09-23, and the same machine now runs aias (`zyx1121/aias`), an installer that puts Docker, Ollama and vLLM into one WSL distro with an MCP server whose tools pull, start and stop a model, one engine on the GPU at a time. aias is the hand made shape of M13 and M15 on one machine, and what it measured is in the `serve-vllm` recipe of 2.2: a 30.7 GB image, 81 s from start to ready and 40 s once the compile cache is a volume of its own, and a default memory fraction that refuses to start on a card anything else touches. More models of that shape are coming, embeddings, rerankers, chat models, and the organization has several GPU machines. The question is whether they become Processes. They do, in three milestones that begin after M12, and each one keeps the four objects and the two built ins. M12 comes first because M11 found it: a Package that is two services.

**What already fits.** A model server is an `http` Process: an image, an environment, a port, a health path and limits. The proxy gives it `<name>.<member>.<domain>`, `secrets` carries its API key, the scheduler can start and stop it, and every request it serves is a span. What is missing is three things: a GPU inside the container, more than one machine, and routing by the `model` field of a request rather than by host name. Which engine serves a model and how it is configured is not among them: that is a skill, see 2.2, so the core grows `limits.gpu` and `volumes` and nothing that knows what Ollama or vLLM is.
...
Two things these milestones do not do. They do not put a Windows host in a cluster: the aias machine stays outside until its GPU runs a Linux node, and a PVE runner kit remains the way to treat a VM as a Process. And they do not add a fifth object: a volume and a node are attributes of a unit and of a placement, and the gateway is a Package.
...
### 5.7 After version 1: skills

**M16 Skills.** Skills need nothing from M13 to M15 and can land before them. The five recipes of 2.2 are seeded under `/org/skills`, the instructions name the folder, and the M11 bench gains two sentences a recipe answers: "run a small chat model I can call from my laptop, only for me" and "a guestbook that keeps its entries when it restarts". Done when a clean agent given the first sentence reads `serve-ollama` or `serve-llamacpp` and the Process it starts refuses a request without the key, given the second it writes a Postgres unit whose rows survive `proc_stop` and `proc_run` with nothing of the database committed, and the same two sentences run against a host without `/org/skills` sit beside them in the bench report, tokens and calls, so what a recipe saves is a number and not a claim.
