...
The boot restore does the same. A Process that is still running is read through the process it already has, because its namespace is the only witness of what it holds and this daemon did not make it. One that is not running is not started by name, because a start makes the bind mounts again and the entrypoint would be running before anything could read them: it is made again through the same four steps. The heal path is the same four steps too. What all of this protects is the `/org` approval trail and progressive disclosure.
