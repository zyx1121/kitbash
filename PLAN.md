...
**The window between the check and the mount is closed after the start, not written down.** podman resolves the source path itself, in its own process, after kitbashd has looked at it, so a source replaced by a symlink in between is followed by podman and the container is given whatever it pointed at. That is not a small hole: an admin could point a `rw` mount at `/org` that way and write the shared root with no approval behind it, and the approval queue is the trail of every change there. So kitbashd reads the container back as soon as it exists. It asks the runtime which source is mounted at each target and in which mode, and it stats each target through `/proc/<pid>/root`, which resolves in the container's own mount namespace, and holds the device and inode it finds there to the ones the validation opened. A mismatch is `not-permitted` with the detail "the mount source changed between validation and start"; the container is stopped and removed rather than left running, and the same check runs in the boot restore and in the heal path. What the check protects is the `/org` approval trail and progressive disclosure.
