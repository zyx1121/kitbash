...
`fs_list` and `packages`, and the paths `/org` and `/home/*`.
...
members or approve anything, whatever a graph asks it to do. A step that names
a tool outside that, or a path or graph outside the two prefixes, is refused
with `not-permitted` naming the step, before the tool runs. The fix is a line in
this kit's manifest rather than a change to your graph.
